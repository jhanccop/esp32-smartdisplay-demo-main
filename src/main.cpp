...
String DateTime = "dd/mm/aaaa hh:mm";
String Diagnosis = "--";
String FillPump = "--";
String SPM = "--";
String PeakLoad = "--";
...
void mqttCallback(char *topic, byte *payload, unsigned int length)
{
    ...
    if (String(topic) == topicSubscribe + WellName + "/data")
    {
        ...
        String WellNames = doc["well"];
        WellName = WellNames;
        String DateTimes = doc["DT"];
        DateTime = DateTimes;
        String DiagnosisA = doc["diagnosis"];
        ...
        for (uint32_t i = 0; i <= numberDataFill; i++)
        {
            ...
            // Serial.println(String(FillPumpList[i]) + String(dtFillPumpList[i]));
        }

        int16_t numberData = 0;
        if(doc["status"] == "running"){
            String str_pos_bottom = doc["pos"];
            String str_load_bottom = doc["load"];
            numberData = s.separa(str_pos_bottom, ',', 0).toInt();
            String SPMs = doc["SPM"];
            SPM = SPMs;
            for (uint32_t i = 1; i <= numberData; i++)
            {
                pos_bottom[i] = s.separa(str_pos_bottom, ',', i).toFloat();
                load_bottom[i] = s.separa(str_load_bottom, ',', i).toFloat();
            }
        }
...
static void add_data(lv_timer_t *timer)
{
    ...
    filter_value_pos = (alpha * ADS.readADC(0) * f) + ((1 - alpha) * filter_value_pos);
    filter_value_load = (alpha * (ADS.readADC(1) - 7290)) + ((1 - alpha) * filter_value_load);
...
void AcquisitionScreen()
{
    ...
    lv_chart_set_range(loadChart, LV_CHART_AXIS_PRIMARY_Y, 0, 2000);
    ...
    lv_chart_set_range(posChart, LV_CHART_AXIS_PRIMARY_Y, 150, 400);
...
void OperationScreen(int16_t numberData, int16_t numberDataFill)
{
    ...
    lv_chart_set_range(dynachart, LV_CHART_AXIS_PRIMARY_X, 0, 420);
    lv_chart_set_range(dynachart, LV_CHART_AXIS_PRIMARY_Y, 0, 600);
...
void setup()
{
    ...
    //Serial.println(String(maxPos) + "," + String(minPos));
    ...
    // ======== DETECT PUMP STOPPED ========
    if (abs(maxPos - minPos) <= 5)
    {
        String mqtt_payload = "{\"well\":\"" + WellName + "\",\"status\":\"stopped\"}";
    ...
    else
    {
        ...
        PeakLoad = String(rload.get(i_start-1), 2);
        MinLoad = String(rload.get(i_flag-1), 2);
        //SPM = String(60 * double(end_time) * 0.001 / (double(read_n_max) * double(i_end - i_start)));
        ...
        {
            rawdataPos += String(rpos.get(i), 2) + ",";
            rawdataLoad += String(rload.get(i), 2) + ",";
        }
        rawdataPos += String(rpos.get(i_end), 2);
        rawdataLoad += String(rload.get(i_end), 2);
        String mqtt_payload = "{\"well\":\"" + WellName + "\",\"status\":\"running\"" + ",\"pos\":\"" + rawdataPos + "\"" + ",\"load\":\"" + rawdataLoad + "\"}";
        sendData(mqtt_payload);
    }
    // sendData("-<-<-<-<-<-mqtt_payload");
    // OperationScreen();
